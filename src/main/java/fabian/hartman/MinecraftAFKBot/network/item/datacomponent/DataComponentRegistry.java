...
import fabian.hartman.MinecraftAFKBot.network.item.datacomponent.components.*;
import fabian.hartman.MinecraftAFKBot.network.item.datacomponent.components.parts.fireworks.FireworkExplosion;
...
public class DataComponentRegistry {
    ...
    public DataComponentRegistry() {
        Registry<Integer, String> dataComponentRegistry = Registries.DATA_COMPONENT_TYPE.getRegistry(MinecraftAFKBot.getInstance().getCurrentBot().getServerProtocol());
        ...
        dataComponentRegistry.forEach((id, name) -> {
            if (!registry.containsKey(id))
                MinecraftAFKBot.getLog().severe("Unhandled data component " + id + " " + name);
        });
    }

    private void addToRegistry(Integer id, Function<Integer, DataComponent> typeToComponentFunction) {
        if (id == null) {
            MinecraftAFKBot.getLog().severe("Tried to add null to data component registry");
            return;
        }
