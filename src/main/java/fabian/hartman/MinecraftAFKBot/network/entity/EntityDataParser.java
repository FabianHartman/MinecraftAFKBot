    ...
    public void readEntityData(ByteArrayDataInputWrapper in, NetworkHandler networkHandler, int protocolId) {
        ...
        try {
            ...
            while (true) {
                ...
                if (MinecraftAFKBot.getInstance().getCurrentBot().getConfig().isLogEntityData())
                    MinecraftAFKBot.getLog().info("Entity data for " + entityId + " is: " + elementIndex + " of type " + elementType + " = " + element.getValue());
            }
            MinecraftAFKBot.getInstance().getCurrentBot().getEventManager().callEvent(new EntityDataEvent(entityId, data));
        } catch (Exception ex) {
            if (MinecraftAFKBot.getInstance().getCurrentBot().getConfig().isLogEntityData())
                ex.printStackTrace();
        }
