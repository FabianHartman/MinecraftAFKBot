...
public class EntityDataElementRegistry {
        ...
        add(ProtocolMapperBuilder.create(5)
                ...
                .build(), simple("slot", (in, networkHandler, protocolId) -> {
                    if (MinecraftAFKBot.getInstance().getConfig().isLogItemData()) {
                        MinecraftAFKBot.getLog().info("Start reading entity data slot");
                    }
                    Slot slot = Packet.readSlot(in, protocolId, networkHandler.getDataComponentRegistry());
                    if (MinecraftAFKBot.getInstance().getConfig().isLogItemData()) {
                        MinecraftAFKBot.getLog().info("End of reading entity data slot");
                    }
                    return slot;
        }));
    ...
    private int readParticle(ByteArrayDataInputWrapper in, NetworkHandler networkHandler, int protocolId) {
        ...
        switch (particleName) {
            ...
            case "minecraft:item":
                if (MinecraftAFKBot.getInstance().getConfig().isLogItemData()) {
                    MinecraftAFKBot.getLog().info("Start reading particle item");
                }
                Packet.readSlot(in, protocolId, networkHandler.getDataComponentRegistry());
                if (MinecraftAFKBot.getInstance().getConfig().isLogItemData()) {
                    MinecraftAFKBot.getLog().info("End of reading particle item");
                }
                break;
