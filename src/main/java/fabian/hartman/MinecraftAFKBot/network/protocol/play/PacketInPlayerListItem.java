...
import fabian.hartman.MinecraftAFKBot.event.play.PingChangeEvent;
        ...
        if (protocolId <= ProtocolConstants.MC_1_19_1) {
            ...
            for (int i = 0; i < playerCount; i++) {
                ...
                if (action == 0) {        //ADD
                    ...
                    if (uuid.equals(MinecraftAFKBot.getInstance().getCurrentBot().getPlayer().getUuid()))
                        MinecraftAFKBot.getInstance().getCurrentBot().getEventManager().callEvent(new PingChangeEvent(ping));
                } else if (action == 1) {
                    readVarInt(in); // gamemode
                } else if (action == 2) {
                    int ping = readVarInt(in);
            ...
            MinecraftAFKBot.getInstance().getCurrentBot().getEventManager().callEvent(new UpdatePlayerListEvent(UpdatePlayerListEvent.Action.REPLACE, getCurrPlayers()));
        } else {
            ...
            in.skipBytes(in.getAvailable());
            MinecraftAFKBot.getInstance().getCurrentBot().getEventManager().callEvent(new UpdatePlayerListEvent(UpdatePlayerListEvent.Action.ADD, addedPlayers));
        }
    }
