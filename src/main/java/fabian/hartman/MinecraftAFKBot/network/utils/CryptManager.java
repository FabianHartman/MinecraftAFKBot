...
public class CryptManager {
    ...
    private static MessageSignature signMessage(AuthData.ProfileKeys keys, UUID signer, String message, long salt, Instant timestamp) {
        ...
        if (MinecraftAFKBot.getInstance().getCurrentBot().getServerProtocol() >= ProtocolConstants.MC_1_19_3) {
            return new MessageSignature(sign(keys, signature -> {
                try {
                    ...
                    signature.update(Ints.toByteArray(MinecraftAFKBot.getInstance().getCurrentBot().getPlayer().incrementChatSessionIndex())); // MessageLink index
        ...
        } else if (MinecraftAFKBot.getInstance().getCurrentBot().getServerProtocol() >= ProtocolConstants.MC_1_19_1) {
            ...
            return new MessageSignature(sign(keys, signature -> {
                try {
                    Optional<MessageSignature> prevSignature = MinecraftAFKBot.getInstance().getCurrentBot().getPlayer().getLastUsedSignature();
